...
// --- 9. internal_error branch: bounded, request-correlated, redacted logging --

// TestUpdateCompanyHandler_InternalErrorLoggingCorrelatedAndRedacted pins the
// write-site logging contract for PATCH /me/company (same contract the
...
// exactly ONE auxiliary log record ("update company failed") plus the runtime
...
// process-global.
func TestUpdateCompanyHandler_InternalErrorLoggingCorrelatedAndRedacted(t *testing.T) {
	const fixedRequestID = "companies-update-fixed-request-id"
	const auxMsg = "update company failed"
	const completionMsg = "http request completed"

	row := mustCompany(uuid.New())
	cc := identitysecurity.CompanyContext{CompanyID: row.ID, UserID: uuid.New(), Role: valueobjects.OwnerRole}
	boom := errors.New("pq: update failed: postgres://svc:hunter2@db.internal:5432/peopleflow token=abc123")
	repo := &stubUpdateServiceRepo{
		getForUpdateOut: row,
		updateErr:       boom,
	}
	...
	handlers := NewCompanyHandler(usecases.NewCompanyService(repo)).CompanyHandlers()
	r.Patch("/me/company", handlers.UpdateCompany)

	req := httptest.NewRequest(http.MethodPatch, "/me/company", strings.NewReader(`{"name":"Acme"}`))
	...
	if got, _ := aux["method"].(string); got != http.MethodPatch {
		t.Errorf("auxiliary method = %q, want %q", got, http.MethodPatch)
	}
