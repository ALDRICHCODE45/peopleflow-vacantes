...
const closeCompanyJobs = `-- name: CloseCompanyJobs :execrows
UPDATE jobs
SET
    status     = 'closed',
...
func (q *Queries) CloseCompanyJobs(ctx context.Context, companyID uuid.UUID) (int64, error) {
	result, err := q.db.Exec(ctx, closeCompanyJobs, companyID)
	if err != nil {
		return 0, err
	}
