...
- **Budget accounting:** untracked `archguard.go` 95 + `archguard_test.go` 257 = 352 candidate additions; this apply-progress note adds the remainder; total ≤400, no exception used. No tracked file modified; `git status --porcelain` shows only `?? backend/internal/tools/` plus this progress note.
